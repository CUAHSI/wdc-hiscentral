<%@ Page Language="C#" AutoEventWireup="true"  CodeFile="pub_Services.aspx.cs" Inherits="pub_services" %>
    ...
    <form id="form1" runat="server">
       <uc1:HeaderControl ID="HeaderControl1" runat="server" />
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:CentralHISConnectionString %>"
            SelectCommand="SELECT NetworkID, username, NetworkName, ServiceWSDL, ContactName, Organization, website, IsPublic, NetworkTitle, ProjectStatus, CONVERT(VARCHAR(10), CreatedDate, 102) AS CreatedDate, CONVERT(VARCHAR(10), case when (networkid=3) then (DATEADD(DAY, -120, GETDATE())) ELSE EarliestRec END, 102) AS EarliestRec, CONVERT(VARCHAR(10), (case WHEN LatestRec>GETDATE()THEN GETDATE() WHEN (networkid = 3) THEN GETDATE() WHEN (DATEADD(DAY, 3, LatestRec) > lastharvested) THEN GETDATE()  ELSE latestRec END) , 102) AS LatestRec  FROM HISNetworks WHERE (IsPublic = 'True') order by servicegroup, networkname">
        </asp:SqlDataSource>
        <div class="mainblock">

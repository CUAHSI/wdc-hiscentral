...
namespace MultiEndpoint
{
    class HealthDataValue
    {
        ...
        //        0: records unchanged
        public int[] HandleDataValueInfoSplit(int scID, int newID, WaterML11.TimeSeriesResponseType[] tsRtAll)
        {
            //Console.WriteLine("Parsing and inserting data value info.");

            WaterML11.ValueSingleVariable dvInfo;
            ...
            {
                hasChanged[iflag] = 0;
                    if (tsRtAll[iflag].timeSeries[0].values[0].value == null)
                    {
                        valueCount[iflag] = 0;
                    }
                    else
                    {
                        valueCount[iflag] = tsRtAll[iflag].timeSeries[0].values[0].value.Count();
                    }
                ...
                if (valueCount[iflag] == 1 &&
                    (double)tsRtAll[iflag].timeSeries[0].values[0].value[0].Value == tsRtAll[iflag].timeSeries[0].variable.noDataValue) 
                {
                ...
                for (int j = 0; j < limit; j++)
                {
                    if (iflag < 2)
                    {
                        if (j >= valueCount[iflag])
                            break;
                        dvInfo = tsRtAll[iflag].timeSeries[0].values[0].value[j];
                        //WaterXML 1.0
                    ...
                    else
                    {
                        if (valueCount[iflag] - 1 - j >=0)
                            dvInfo = tsRtAll[iflag].timeSeries[0].values[0].value[valueCount[iflag] - 1 - j];
                        else
                            break;
                    }
        ...
        public int[] HandleDataValueInfo(int scID, int newID, WaterML11.TimeSeriesResponseType tsRt)
        {
            //Console.WriteLine("Parsing and inserting data value info.");
            ...
            if (valueCount == 1 &&
                tsRt.timeSeries[0].values[0].value[0].Value == -99)
            {
                Console.WriteLine("No values in WS response.");
                // there are values in SC, but not in the response from WS
            }
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (int j = 0; j < limit; j++)
                {
                    dvInfo = tsRt.timeSeries[0].values[0].value[idx + j];
